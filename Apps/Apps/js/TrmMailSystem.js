        ...
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                ...
                if (json[i].TicketNumber == "") {
                    var urlticket = "<a class='dropdown-item' href='#' onclick=CrateTicket('" + json[i].IVC_ID + "')><i class='si-arrow-right-circle si'></i> Ticket</a>"
                } else {
                    var urlticket = ""
                }
...
$(document).on("change", "input[name='files']", function (e) {
    ...
    for (var i = 0; i < files.length; i++) {
        ...
        if (fileextension == "xls" || fileextension == "xlsx" || fileextension == "mp4" || fileextension == "Mp4" || fileextension == "Mp3" || fileextension == "mp3" || fileextension == "doc" || fileextension == "docx" || fileextension == "pdf" || fileextension == "png" || fileextension == "PNG" || fileextension == "jpg" || fileextension == "JPG" || fileextension == "jpeg" || fileextension == "gif" || fileextension == "GIF" || fileextension == "bmp" || fileextension == "BMP") {
...
function ReplaceEmailCC(AddressEmail) {
    ...
    newString = dummyString.replace('care@cantas.co.id;', '').replace('care@cantas.id;', '');
    //alert(newString);
    return newString
}
function EmailConversation(RefID) {
    ...
    $.ajax({
        ...
        data: "{RefID: '" + ReplaceConversation(RefID) + "', UserName: '" + $("#hd_sessionLogin").val() + "', Action: 'SELECT'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
            ...
            for (i = 0; i < json.length; i++) {
                if (json[i].DIRECTION == "IN") {
                    ...
                    await $.ajax({
                        ...
                        data: "{RefID: '" + ReplaceConversation(RefID) + "', EmailID: '" + json[i].EMAIL_ID + "', Direction: 'IN', UserName: '" + $("#hd_sessionLogin").val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (data) {
                            var json = JSON.parse(data.d);
                    ...
                    await $.ajax({
                        type: "POST",
                        url: "asmx/TrmMailConversation.asmx/UIDESK_TrxEmailAttachment",
                        data: "{RefID: '" + ReplaceConversation(RefID) + "', EmailID: '" + json[i].EMAIL_ID + "', Direction: 'OUT', UserName: '" + $("#hd_sessionLogin").val() + "'}",
                        contentType: "application/json; charset=utf-8",
                        dataType: "json",
                        success: function (data) {
                            var json = JSON.parse(data.d);
...
function CrateTicket(ParamID) {
    $.ajax({
        type: "POST",
        url: "WebServiceGetDataMaster.asmx/UIDESK_TrmMasterCombo",
        data: "{TrxID:'EMAIL-" + ParamID + "', TrxUserName: '" + $("#hd_sessionLogin").val() + "', TrxAction: 'UIDESK119'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            ...
            for (i = 0; i < json.length; i++) {
                location.href = "1_doticket.aspx?id=" + json[i].CustomerID + "&channel=Email&n=1&threadid=" + json[i].ThreadID + "&numberid=" + json[i].GenesysNumber + "&account=" + json[i].Account + ""
            }
...
function ReplaceConversation(RefID) {
    let dummyString = RefID;
    newString = dummyString.replace('%20', ' ').replace('%20', ' ');
    //alert(newString)
    return newString
}